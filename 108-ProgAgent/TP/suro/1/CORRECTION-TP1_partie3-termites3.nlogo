turtles-own [etape fatigue] ;; un attribut pour représenter l'étape du comportement de la termite
...
to setup
  ...
  [ if random-float 100 < density
    [ set pcolor yellow ]
    if random-float 100 < density ;; ajouter des brindilles vertes
    [ set pcolor green ]
  ]
...
to find-new-pile  ;; turtle procedure -- look for yellow patches
  ifelse pcolor = color and verifier-distance
  [ set etape "poser"]
  [ wiggle
     ]
end
...
to put-down-chip  ;; turtle procedure -- finds empty spot & drops chip
  ifelse pcolor = black and verifier-distance
  [
    set pcolor color
    set color white
    set etape "fuir" ]
...
to-report verifier-distance
  if fatigue = 0
  [report true]
  ifelse color = green
  [
    if any? patches in-radius fatigue with [pcolor = yellow]
    [
      set fatigue fatigue - 0.1
      report false
    ]
  ]
  [
    if any? patches in-radius fatigue with [pcolor = green]
    [
      set fatigue fatigue - 0.1
      report false
    ]
