...
// file used for the key : ./ipc
	...
	if (cle ==-1) {
		perror("Serveur DIV  erreur ftok");
		exit(1);
	}

	printf("Serveur DIV  : Clé généré avec succès :)\n");
	...
	if(msgid == -1) {
		perror("Serveur DIV  erreur msgget");
		exit(1);
	}

	printf("Serveur DIV  : File de message accédé ou crée avec succès :D\n");
	...
	while(flag == true){
		printf("Serveur DIV  : En attente de message à traiter (:\n");
		if(msgrcv(msgid, (void *)&maRequete, sizeof(maRequete.contenu), '/', 0) == -1){
			perror("Serveur DIV  erreur msgrcv");
			exit(1);
		}else{
			printf("Serveur DIV : opération reçue  => %d %c %d\n", maRequete.contenu.op1, (char) maRequete.etiqReq, maRequete.contenu.op2);
			printf("Serveur DIV  : Message en traitement :)\n");
			int res = 0;
			...
			if(maRequete.contenu.flag == false){
				flag = false;
				printf("Serveur DIV  : Message d'arret reçu :/\n");
			}
			...
			if(msgsnd(msgid, (void *)&maRequete, sizeof(maRequete.contenu), 0) == -1){
				perror("Serveur DIV  erreur msgsnd");
				exit(1);
			}
			printf("Serveur DIV  : Réponse envoyé :D\n");
		}
	}
	printf("Serveur DIV : Je termine ...\n");
	return 0;
}