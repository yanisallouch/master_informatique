...
void *fnt_envoiRequette(void * param){
	...
	int ville = 0;
	int update = 1;
	int cpu, stockage, partager, liberation;
	struct reservation bufReservation;
	pthread_mutex_lock(&verrou);
	pthread_cond_wait(&condi,&verrou);
	pthread_mutex_unlock(&verrou);
	while(ville != -1){
		printf("Update l'affichage 0/1 ?\n" );
		scanf("%d", &update );
		if(update){
			affichageRessources(ptrShm->sitesDispo, taille);
		}else{
			printf("Client : Saisi clavier idVille >=0 CPU >0 Stockage >0 partager 0/1 liberation 0/1 ?\n");
