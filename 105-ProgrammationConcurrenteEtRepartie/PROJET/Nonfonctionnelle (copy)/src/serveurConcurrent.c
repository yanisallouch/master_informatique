...
void initSitesManuel(struct sites * sitesDispo, int* taille);
...
void initSitesManuel(struct sites * sitesDispo, int* taille){
	...
	memset(sitesDispo, 0, (*taille)*sizeof(struct sites));
	printf("Entrez succesivement le nom, le nombre de processeurs et le nombre de Gio de stockage par sites\n" );
...
// TODO HISTORIQUE DES REQUETES
	...
	while(flagExit == 0){
		...
		if(bufReservation.label != -1){
			if(bufReservation.estPartage){
				//partage
				if(bufReservation.estLiberation){
					// TODO ??
				}else{
					P(semId,bufReservation.label);
					if(ptrShm->sitesDispo[bufReservation.label].nbProcesseurPartage < bufReservation.nbProcesseur){
					...
					if(ptrShm->sitesDispo[bufReservation.label].nbCapaciteStockagePartage < bufReservation.nbCapaciteStockage){
