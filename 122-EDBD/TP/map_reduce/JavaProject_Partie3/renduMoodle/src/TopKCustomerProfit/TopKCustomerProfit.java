...
import TopKLignesProfit.BigDecimalWritable;
...
class Map extends Mapper<LongWritable, Text, BigDecimalWritable, Text> {
	...
	public void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
		...
		context.write(new BigDecimalWritable(profit), value);
...
class Reduce extends Reducer<BigDecimalWritable, Text, BigDecimalWritable, Text> {
	/**
	 ...
	 */
	private TreeMap<BigDecimalWritable, List<Text>> sortedLines = new TreeMap<>();
	private int nbSortedLines = 0;
	private int k;
	...
	@Override
	public void reduce(BigDecimalWritable key, Iterable<Text> values, Context context)
			throws IOException, InterruptedException {
		
		BigDecimalWritable keyCopy = new BigDecimalWritable(key.get());
		
		for (Text val: values) {
			
			// Profit déjà présent
			if (sortedLines.containsKey(key)) {
				sortedLines.get(keyCopy).add(val);
			}
			else {
				List<Text> lignes = new ArrayList<>();

				lignes.add(val);
				assert(lignes.size() == 1);
				sortedLines.put(keyCopy, lignes);
			}

			// Nombre de lignes enregistrées atteint : on supprime la ligne avec le plus petit profit (la première dans sortedWords)
			if (nbSortedLines == k) {
				BigDecimalWritable firstKey = sortedLines.firstKey();
//				System.out.println("Key is "+ firstKey);
//				System.out.println("List is size "+ sortedLines.get(firstKey).size() );
				List<Text> lines = sortedLines.get(firstKey);
				
				int sizeBefore = lines.size();
				
				lines.remove(lines.size() - 1);
				//System.out.println("List is now size "+ sortedLines.get(firstKey).size() );
				assert(lines.size() == sizeBefore -1);
				if (lines.isEmpty())
					sortedLines.remove(firstKey);
			} else
				nbSortedLines++;
	...
	public void cleanup(Context context) throws IOException, InterruptedException {
		int i = 0;
		System.out.println(sortedLines.size());
		for (BigDecimalWritable d : sortedLines.descendingKeySet()) {
			if (i == k) break;
			for (Text line: sortedLines.get(d)) {
				if (i == k) break;
				context.write(d, line);
				i++;
			}
		}
	}
...
public class TopKCustomerProfit {
	private static final String INPUT_PATH = "input-groupBy";
	private static final String OUTPUT_PATH = "output/TopkLignesProfit-";
	private static final Logger LOG = Logger.getLogger(TopKCustomerProfit.class.getName());
	...
	public static void main(String[] args) throws Exception {
		...
		Job job = new Job(conf, "Top K Lines Profit");

		job.setOutputKeyClass(BigDecimalWritable.class);
