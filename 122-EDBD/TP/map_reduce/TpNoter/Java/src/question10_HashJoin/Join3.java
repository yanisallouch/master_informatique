		...
		public void reduce(Text key, Iterable<Text> values, Context context)
			...
			for (Text text : values) {
				...
				if (columns2.length == 3) {
					produitsMoyennes.add(new Pair<Integer, Double>(Integer.parseInt(columns2[0]),Double.parseDouble(columns2[1])));
					sumTotalContinent += Double.parseDouble(columns2[1].trim());
					qteTotal += Integer.parseInt(columns2[2].trim());
				}
			}
			for (Pair<Integer, Double> pair : produitsMoyennes) {
				if (Double.parseDouble(String.valueOf(pair.second)) >= (sumTotalContinent/qteTotal) ) {
					context.write(new Text(nomContinent), new Text("|" + pair.toString() + ", " + (sumTotalContinent/qteTotal)));
				}
			}
		}
	}
	
	private static final String INPUT_PATH = "output/input-output-hashjoin-10-2"; // dimensions
	private static final String OUTPUT_PATH = "output/input-output-hashjoin-10-3-";
...
géographique ?
	...
	public static void main(String[] args) throws Exception {
		...
		Job job = new Job(conf, "Join HashJoin Input-10-2 Continent Lieux");
