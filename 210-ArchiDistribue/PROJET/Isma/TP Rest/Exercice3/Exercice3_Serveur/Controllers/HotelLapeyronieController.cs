...
{
    public class HotelLapeyronieController : ApiController
    {
        private static Hotel hotelLapeyeronie;
        private static List<Offer> listAllOffres;
        private string login = "lapeyronie";
        private string password = "lapeyronie";
        private static bool isAuth = false;
        private static bool isOfferExist = false;

        [Route("HotelLapeyronie/Authtification")]
        public bool Get(string login, string password)
        {
            if (login.Equals(this.login) && password.Equals(this.password))
            {
                ...
                if (hotelLapeyeronie == null || listAllOffres == null)
                {
        ...
        [Route("HotelLapeyronie/GetListeOffre")]
        public string Get(string dateDebut, string dateFin, int nombrePersonne)
        {
            if (isAuth)
            {
                string listOffers = "";
        ...
        [Route("HotelLapeyronie/VerifyOffer")]
        public string Get(string idOffer)
        {
            for (int i = 0; i < listAllOffres.Count; i++)
            {
                Offer offer = listAllOffres.ElementAt(i);
        ...
        [Route("HotelLapeyronie/BookingRegistration")]
        public string Get(string idOffre, int client)
        {
            if (isOfferExist)
            {
                for (int i = 0; i < listAllOffres.Count; i++)
        ...
        [Route("HotelLapeyronie/CreateHotelAndGenerateOffers")]
        public void CreateHotelAndGenerateOffers()
        {
            ...
            hotelLapeyeronie = new Hotel("Hotel Laperonie",
                                         "France",
                                         "Montpellier",
                                         "570 Route de Ganges",
                                         "Lapeyronie",
            ...
            for (int i = 0; i < hotelLapeyeronie.ListeChambre.Count; i++)
            {
                Offer offer = new Offer();

                if (i < 10)
                    offer = new Offer("Offre$" + (i + 1), hotelLapeyeronie.ListeChambre.ElementAt(i), "2020-03-0" + (i + 1), hotelLapeyeronie.ListeChambre.ElementAt(i).PrixChambre - 20, hotelLapeyeronie.ListeChambre.ElementAt(i).ImageURL, true);
                else
                    offer = new Offer("Offre$" + (i + 1), hotelLapeyeronie.ListeChambre.ElementAt(i), "2020-03-" + (i + 1), hotelLapeyeronie.ListeChambre.ElementAt(i).PrixChambre - 20, hotelLapeyeronie.ListeChambre.ElementAt(i).ImageURL, true);
